...
Boar::Boar(Vector2f position) : Enemy (BOAR_HP) {
	...
	hitbox->setSize(Vector2f(sprite.getLocalBounds().width*hitbox_sprite_xRatio, sprite.getLocalBounds().height*hitbox_sprite_yRatio));
    hitbox->setFillColor(sf::Color(255, 0, 0, 128));
	hitboxes.push_back(hitbox);

    sf::RectangleShape* zhitbox = new sf::RectangleShape();
	zhitbox->setSize(Vector2f(hitbox->getSize().x * hitbox_zHitbox_xRatio, hitbox->getSize().y * hitbox_zHitbox_yRatio));
