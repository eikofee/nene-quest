...
Arrow::Arrow(Vector2f position, float g_depth) {
    ...
    hitbox->setSize(
        Vector2f(texture.getSize().x / 5, texture.getSize().y / 1.8));
...
void Arrow::update(float elapsedTime) {
    if (!is_dying) {
        ...
        /* this->setPosition(initial_location.x +
         lifespan*speed.x,initial_location.y + lifespan*speed.y +
         gravity*lifespan/2); sprite.setRotation(atan((speed.y +
         gravity*lifespan)/speed.x)*180/3.1415);
     ...
     */
        ...
        this->setPosition(
            initial_location.x + speed.x * lifespan,
            initial_location.y +
                speed.y * (sin((lifespan / curveTime) *
                               M_PI) /*+ sin(curveCappedValue * M_PI)*/));
        zHitboxes.at(0)->setPosition(getPosition().x, initial_location.y);
        sprite.setRotation(
            -30 +
            lifespan * 60);  // rotation start + (rotation end - rotation start)
        hitboxes.at(0)->setRotation(sprite.getRotation());
        if (lifespan >= highDeadzoneStart * curveTime &&
            lifespan <= highDeadzoneEnd * curveTime)
            zHitboxes.at(0)->setFillColor(
                sf::Color(255, 128, 255, 128));  // pink
        else
            zHitboxes.at(0)->setFillColor(sf::Color(0, 0, 255, 128));  // blue

        if (lifespan >= curveTime) is_dead = true;
    } else {
...
bool Arrow::isDead() { return is_dead; }

void Arrow::kill() {
    this->depth = 8000;
    damage_sprite.setRotation(hitboxes.at(0)->getRotation());
    damage_sprite.setPosition(hitboxes.at(0)->getPosition().x,
                              hitboxes.at(0)->getPosition().y);
    is_dying = true;
}
...
void Arrow::draw(sf::RenderTarget &target, sf::RenderStates states) {
    ...
    if (is_dying) target.draw(damage_sprite, states);
}
