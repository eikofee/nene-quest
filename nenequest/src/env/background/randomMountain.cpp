...
RandomMountain::RandomMountain() {
    ...
    this->body->setOrigin(this->body->getSize().x / 2,
                          this->body->getSize().y / 2);
    this->body->setRotation(45 + angle);
}

RandomMountain::~RandomMountain() { delete this->body; }
...
bool RandomMountain::isAlive() { return this->alive; }

void RandomMountain::translate(float x, float y) { this->body->move(x, y); }
