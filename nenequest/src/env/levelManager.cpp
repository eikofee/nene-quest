...
void LevelManager::setLevelCommands(std::vector<ParserCommand *> commands) {
    for (auto c : commands) this->levelCommands.push_back(c);
...
void LevelManager::update() {
    if (!isWaiting && this->levelCommands.size() > 0)
        if (this->levelCommands.front()->getTime() <=
            (this->timeSync + this->clock.getElapsedTime().asSeconds())) {
...
// Game setters
void LevelManager::setBackground(Background *bck) { this->background = bck; }
